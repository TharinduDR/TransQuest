...
class GoogleDriveDownloader:
    ...
    def download_file_from_google_drive(file_id, dest_path, overwrite=False, unzip=False, showsize=False):
        ...
        if not exists(dest_path) or overwrite:
            ...
            logging.info('Downloading {} into {}... '.format(file_id, dest_path), end='')
            stdout.flush()
        ...
        with open(destination, 'wb') as f:
            for chunk in response.iter_content(GoogleDriveDownloader.CHUNK_SIZE):
                if chunk:  # filter out keep-alive new chunks
                    f.write(chunk)
                    if showsize:
                        logging.info('\r' + GoogleDriveDownloader.sizeof_fmt(current_size[0]), end=' ')
                        stdout.flush()
